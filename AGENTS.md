...
- `version` constant in `constants.ts` shows in the header. It follows `yyyy.mm.dd<suffix>` and is bumped automatically by `npm run build` (see "Release artifact" above). Do not edit it manually.
